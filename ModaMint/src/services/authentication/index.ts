...
apiClient.interceptors.request.use(
  (config) => {
    // KHÔNG thêm token vào request login, logout, refresh, google oauth
    const isAuthRequest =
  ...
  async (error) => {
    ...
    if (error.response?.status === 401 && !originalRequest._retry) {
      ...
      try {
        ...
        const refreshResult = await authenticationService.refreshAccessToken(
        ...
        if (refreshResult.success && refreshResult.data) {
          ...
          const newAuthData = {
            ...
            refreshToken:
              refreshResult.data.refreshToken || authData.refreshToken,
          };
...
class AuthenticationService {
  // Method đăng nhập với Google OAuth
  async authenticateWithGoogle(code: string): Promise<{
    ...
    try {
      console.log(
        "🔐 Attempting Google OAuth login with code:",
        code.substring(0, 20) + "..."
      );
      console.log(
        "🌐 Request URL:",
      ...
      if (apiResponse.code !== undefined) {
        ...
        console.log("✅ Google OAuth successful (ApiResponse format)");
        return {
          success: true,
      ...
      if ((apiResponse as any).accessToken) {
        console.log("✅ Google OAuth successful (Direct format)");
        return {
          success: true,
      ...
      if (axios.isAxiosError(error)) {
        ...
        return {
          ...
          message:
            ...
            `Đăng nhập Google thất bại (${error.response?.status})`,
        };
      }
  ...
  // Method đăng nhập
  async authenticate(credentials: AuthenticationRequest): Promise<{
    success: boolean;
    data?: AuthenticationResponse;
    message?: string;
  }> {
    ...
    try {
      ...
      if (axios.isAxiosError(error)) {
        ...
        return {
          success: false,
          message:
            errorResponse?.message || "Không thể lấy thông tin người dùng",
        };
      }
      return {
        success: false,
